...
async function initAuth() {
  try {
    ...
    $('login').onclick = async () => {
      ...
      try {
        ...
        if (!OWNER_EMAILS.includes(email) || !result.user.emailVerified) {
          $('state').textContent =
            `${email || 'This account'} is not an allowed owner account.`;
        ...
        $('state').textContent =
          `Sign-in failed: ${friendlyError(err)}`;
      }
    };
    ...
    $('state').textContent =
      `Firebase failed to load: ${friendlyError(err)}`;
  }
}
...
function friendlyError(err) {
  ...
  if (code.includes('unauthorized-domain')) {
    return 'This website domain is not added in Firebase Authentication → Settings → Authorized domains.';
  }
...
async function openDashboard() {
  ...
  try {
    ...
    $('state').textContent =
      `Could not open dashboard: ${friendlyError(err)}`;
    ...
    Object.entries(labels).map(([key, label]) =>
      `<div>
        ...
        <input
          type="number"
          id="bv-${key}"
          value="${num(currentConfig.baseValues[key])}"
          min="1"
          max="200"
        >
      </div>`
...
function renderRam() {
  $('ram-grid').innerHTML =
    ['4', '6', '8', '12'].map(ram =>
      `<div>
        <label>${ram === '12' ? '12 GB+' : ram + ' GB'}</label>
        <input
          type="number"
          id="ram-${ram}"
          value="${num(currentConfig.ramAdj[ram])}"
        >
      </div>`
    ).join('');
}
...
function renderBrands() {
    ...
    brands.map((brand, i) =>
      `<div class="item-row editable">
        ...
        <input
          type="number"
          data-brand-adj="${i}"
          value="${num(currentConfig.brandAdj[brand])}"
        >
  ...
  $('brands-list')
    .querySelectorAll('[data-remove-brand]')
    .forEach(btn => {
      btn.onclick = () => {
        ...
        currentConfig.phoneModels =
          currentConfig.phoneModels.filter(
            m => m.brand !== brand
          );
  ...
  $('brand-add').onclick = () => {
    ...
    if (!name || currentConfig.phoneBrands.includes(name)) {
      return;
    }

    currentConfig.phoneBrands.push(name);

    currentConfig.brandAdj[name] =
      Number($('brand-adj').value || 0);
...
function fillModelBrandSelect() {
  ...
  sel.innerHTML =
    '<option value="">Any / no brand</option>';
  ...
  sel.value =
    currentConfig.phoneBrands.includes(prev)
      ? prev
      : '';
}
...
function renderModels() {
  ...
  $('models-list')
    .querySelectorAll('[data-remove-model]')
    .forEach(btn => {
      btn.onclick = () => {
        currentConfig.phoneModels.splice(
          Number(btn.dataset.removeModel),
          1
        );
...
function renderTips() {
  $('tip-rusher').value =
    currentConfig.tips.rusher || '';

  $('tip-sniper').value =
    currentConfig.tips.sniper || '';

  $('tip-balanced').value =
    currentConfig.tips.balanced || '';
}


/* =========================================================
   🧠 WESSY AI KNOWLEDGE BASE
   ========================================================= */
...
function renderKnowledge() {
  ...
  $('knowledge-list').innerHTML =
    knowledge.map((item, i) =>
      `<div class="item-row knowledge-row">
        <span>
          <b>${escapeHtml(item.topic || '')}</b>
          <small>${escapeHtml(item.information || '')}</small>
        </span>

        <div class="item-actions">
          <button
            data-edit-knowledge="${i}"
            title="Edit knowledge"
          >✎</button>

          <button
            data-remove-knowledge="${i}"
  ...
  $('knowledge-list')
    .querySelectorAll('[data-edit-knowledge]')
    .forEach(btn => {
      btn.onclick = () => {
        const index =
          Number(btn.dataset.editKnowledge);

        const item =
          currentConfig.knowledgeBase[index];
        ...
        const information = prompt(
          'Information Wessy AI should know',
          item.information || ''
        );
        ...
        item.information = information.trim();
  ...
  $('knowledge-list')
    .querySelectorAll('[data-remove-knowledge]')
    .forEach(btn => {
      btn.onclick = () => {
        const index =
          Number(btn.dataset.removeKnowledge);

        currentConfig.knowledgeBase.splice(
          index,
          1
        );
  ...
  $('knowledge-add').onclick = () => {
    const topic =
      $('knowledge-topic').value.trim();

    const information =
      $('knowledge-information').value.trim();

    if (!topic || !information) {
      return;
    }

    currentConfig.knowledgeBase.push({
      topic,
      information
    });
...
/* =========================================================
   💬 LEGACY WESSY AI CHAT REPLIES
   ========================================================= */

function renderReplies() {
  const replies =
    currentConfig.wessyReplies || [];
  ...
  $('replies-list')
    .querySelectorAll('[data-edit-reply]')
    .forEach(btn => {
      btn.onclick = () => {
        const reply =
          currentConfig.wessyReplies[
            Number(btn.dataset.editReply)
          ];
  ...
  $('replies-list')
    .querySelectorAll('[data-remove-reply]')
    .forEach(btn => {
      btn.onclick = () => {
        currentConfig.wessyReplies.splice(
          Number(btn.dataset.removeReply),
          1
        );
  ...
  $('reply-add').onclick = () => {
    ...
    currentConfig.wessyReplies.push({
      keywords,
      reply
    });
...
function collectForm() {
  ...
  ['4', '6', '8', '12'].forEach(k => {
    ramAdj[k] =
      Number($(`ram-${k}`).value || 0);
  });

  const brandAdj = {};

  (currentConfig.phoneBrands || [])
    .forEach((brand, i) => {
      const input =
  ...
  return mergeConfig({
    ...
    tips: {
      rusher:
        $('tip-rusher').value.trim(),

      sniper:
        $('tip-sniper').value.trim(),

      balanced:
        $('tip-balanced').value.trim()
    },
...
$('save').onclick = async () => {
  ...
  try {
    ...
    $('save-status').textContent =
      '✅ Saved. The public Wessy site receives the update in real time.';
  } catch (err) {
    $('save-status').textContent =
      `❌ Save failed: ${friendlyError(err)}`;
  } finally {
    $('save').disabled = false;
  }
};
